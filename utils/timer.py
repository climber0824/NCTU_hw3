...
def disable(fn_name):
    """ Disables the given function name fom being considered
        for the average or outputted in print_stats. """
...
def stop(fn_name=None, use_stack=True):
    ...
    if use_stack:
        ...
        if _start_times[fn_name] > -1:
            _total_times[fn_name] += (
                time.perf_counter() - _start_times[fn_name]
            )
        else:
