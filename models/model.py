...
class PredictionModule(nn.Module):
    ...
    Args:
        ...
        - aspect_ratios:
            A list of lists of priorbox aspect ratios (one list per scale).
        - scales:
            A list of priorbox scales relative to this layer's convsize.
        ...
        - parent:
            If parent is a PredictionModule,
            this module will use all the layers
    ...
    def forward(self, x):
        ...
        Returns a tuple
        (bbox_coords, class_confs, mask_output, prior_boxes) with sizes
    ...
    def make_priors(self, conv_h, conv_w, device):
        ...
        with timer.env("makepriors"):
            if self.last_img_size != (cfg._tmp_img_w, cfg._tmp_img_h):
                    ...
                    for scale, ars in zip(self.scales, self.aspect_ratios):
                        for ar in ars:
                            ...
                            if cfg.backbone.use_pixel_scales:
                                w = (
                                    scale * ar / cfg._tmp_img_w
                                )  # These are populated by
                                h = (
                                    scale / ar / cfg._tmp_img_h
                                )  # Yolact.forward
                            else:
                                w = scale * ar / conv_w
                                h = scale / ar / conv_h
                ...
                self.priors = (
                    torch.Tensor(prior_data).cuda().view(-1, 4).detach()
                )
...
class FPN(ScriptModuleWrapper):
    ...
    Parameters (in cfg.fpn):
        ...
        - num_downsample (int):
            The number of downsampled layers to add onto the selected layers.
    ...
    def forward(self, convouts: List[torch.Tensor]):
        """
        Args:
            - convouts (list): A list of convouts
            for the corresponding layers in in_channels.
        Returns:
            - A list of FPN convouts in the same order as
            x with extra downsample layers if requested.
        """
...
class Yolact(nn.Module):
    """
    You can set the arguments by changing them
    in the backbone config object in config.py.

    Parameters (in cfg.backbone):
        - selected_layers:
        The indices of the conv layers to use for prediction.
        - pred_scales:
        A list with len(selected_layers) containing tuples of scales
        (see PredictionModule)
        - pred_aspect_ratios:
        A list of lists of aspect ratios with len(selected_layers)
        (see PredictionModule)
    """
    ...
    def init_weights(self, backbone_path):
        ...
        for name, module in self.named_modules():
            is_script_conv = (
                "Script" in type(module).__name__
