...
from config import cfg, mask_type, activation_func
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets, masks, num_crowds):
        ...
        for idx in range(batch_size):
            ...
            if cfg.use_class_existence_loss:
                class_existence_t[idx, :] = (
                    torch.eye(num_classes - 1)[labels[idx]]
                    .cuda()
                    .max(dim=0)[0]
                )
        ...
        if cfg.train_boxes:
            ...
            losses["B"] = (
                F.smooth_l1_loss(loc_p, loc_t, reduction="sum")
                * cfg.bbox_alpha
            )
            ...
            if cfg.mask_type == mask_type.direct:
                ...
                if cfg.mask_proto_loss is not None:
                    if cfg.mask_proto_loss == "l1":
                        ...
                        losses["P"] = -torch.mean(
                            torch.max(
                                F.log_softmax(proto_data, dim=-1), dim=-1
                            )[0]
                        )
    ...
    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        ...
        if cfg.use_class_balanced_conf:
            ...
            if self.class_instances is None:
                self.class_instances = torch.zeros(self.num_classes).cuda()
    ...
    def focal_conf_sigmoid_loss(self, conf_data, conf_t):
        ...
        conf_one_t = torch.eye(num_classes)[conf_t].cuda()
        conf_pm_t = (
            conf_one_t * 2 - 1
    ...
    def conf_objectness_loss(
        self, conf_data, conf_t, batch_size, loc_p, loc_t, priors
    ):
        """
    ...
    def direct_mask_loss(
        self, pos_idx, idx_t, loc_data, mask_data, priors, masks
    ):
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                ...
                for jdx in range(num_pos):
                    tmp_mask = pos_masks[
                        jdx, y1[jdx]: y2[jdx], x1[jdx]: x2[jdx]
                    ]
            ...
            with torch.no_grad():
                ...
                if cfg.mask_proto_reweight_mask_loss:
                    ...
                    gt_background_norm = (1 - bin_gt) / (
                        torch.sum(1 - bin_gt, dim=(0, 1), keepdim=True)
                        + 0.0001
                    )
            ...
            if cfg.mask_proto_double_loss:
                if cfg.mask_proto_mask_activation == activation_func.sigmoid:
                    pre_loss = F.binary_cross_entropy(
                        torch.clamp(pred_masks, 0, 1), mask_t, reduction="sum"
                    )
                else:
