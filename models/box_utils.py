...
def intersect(box_a, box_b):
    ...
    B = box_b.size(1)
    max_xy = torch.min(
        box_a[:, :, 2:].cuda().unsqueeze(2).expand(n, A, B, 2),
        box_b[:, :, 2:].cuda().unsqueeze(1).expand(n, A, B, 2),
    )
    min_xy = torch.max(
        box_a[:, :, :2].cuda().unsqueeze(2).expand(n, A, B, 2),
        box_b[:, :, :2].cuda().unsqueeze(1).expand(n, A, B, 2),
...
def jaccard(box_a, box_b, iscrowd: bool = False):
    ...
    out = (
        inter.cuda() / area_a.cuda()
        if iscrowd
        else inter.cuda() / union.cuda()
    )
    return out if use_batch else out.squeeze(0)


def elemwise_box_iou(box_a, box_b):
    """ Does the same as above but instead of pairwise,
    elementwise along the inner dimension. """
...
def mask_iou(masks_a, masks_b, iscrowd=False):
    """
    Computes the pariwise mask IoU between two sets of
    masks of size [a, h, w] and [b, h, w].
    ...
    return (
        ...
        if not iscrowd
        else intersection / area_a
    )


def elemwise_mask_iou(masks_a, masks_b):
    """ Does the same as above but instead of pairwise,
    elementwise along the outer dimension. """
    ...
    return torch.clamp(
        intersection / torch.clamp(area_a + area_b - intersection, min=0.1),
        max=1,
    )
...
def match(
    ...
    Args:
        ...
        crowd_boxes:
        (tensor) All the crowd box annotations or None if there are none.
        loc_t: (tensor) Tensor to be filled w/ endcoded location targets.
        conf_t:
        (tensor) Tensor to be filled w/ matched indices for conf preds.
        Note: -1 means neutral.
        idx_t: (tensor) Tensor to be filled w/ the index of the matched
        gt box for each prior.
        idx: (int) current batch index.
        loc_data:
        (tensor) The predicted bbox regression coordinates for this batch.
    Return:
        The matched indices corresponding to 1)location and 2)confidence preds.
    """
    decoded_priors = (
        decode(loc_data, priors, cfg.use_yolo_regressors)
        if cfg.use_prediction_matching
        else point_form(priors)
    )

    # Size [num_objects, num_priors]
    overlaps = (
        jaccard(truths, decoded_priors)
        if not cfg.use_change_matching
        else change(truths, decoded_priors)
    )
...
def sanitize_coordinates(
    ...
    """
    Sanitizes the input coordinates
    so that x1 < x2, x1 != x2, x1 >= 0, and x2 <= image_size.
    Also converts from relative to absolute coordinates
    and casts the results to long tensors.

    If cast is false, the result won't be cast to longs.
    Warning: this does things in-place behind the scenes
    so copy if necessary.
    """
    _x1 = _x1 * img_size
    _x2 = _x2 * img_size
    if cast:
...
def crop(masks, boxes, padding: int = 1):
    ...
    rows = (
        torch.arange(w, dtype=x1.dtype).cuda().view(1, -1, 1).expand(h, w, n)
    )
    cols = (
        torch.arange(h, dtype=x1.dtype).cuda().view(-1, 1, 1).expand(h, w, n)
    )
...
def index2d(src, idx):
    ...
    offs = torch.arange(idx.size(0))[:, None].cuda().expand_as(idx)
